...
void matrixMultiplyAdd(int*& A, int*& B, int*& C, int size)
{
...
void matrixMultiplicationMPI(int*& localA, int*& localB, int*& localC, int size, ProcInfo* info)
{
	...
	int sendLeftTo = (info->column - info->row + info->dim) % info->dim;
	int sendUpTo = (info->row - info->column + info->dim) % info->dim;

	int recvRightFrom = (info->column + info->row) % info->dim;
	int recvDownFrom = (info->row + info->column) % info->dim;

	MPI_Sendrecv_replace(localA, 1, MPI_INT, sendLeftTo, tag, recvRightFrom, tag, info->rowComm, &status);

	MPI_Sendrecv_replace(localB, 1, MPI_INT, sendUpTo, tag, recvDownFrom, tag, info->columnComm, &status);
	
	sendLeftTo = (info->column - 1 + info->dim) % info->dim;
	sendUpTo = (info->row - 1 + info->dim) % info->dim;

	recvRightFrom = (info->column + 1) % info->dim;
	recvDownFrom = (info->row + 1) % info->dim;

	for (int i = 0; i < info->dim; i++)
	{
		matrixMultiplyAdd(localA, localB, localC, submatrixSize);
